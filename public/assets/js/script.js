...
function getCookie(name) { // name attend le nom du cookie que l'on souhaite récupérer
  ...
  const value = cookies // je récupère tous les cookies
    .find( // je fais un find pour trouver le cookie qui correspond à une fonction
...
function applyDarkmode() {
    if (getCookie('designMode') === '2') { // j'utilise la fonction pour récupérer la valeur d'un cookie et si elle est === 2 alors on est en darkmode
      getDarkMode();
    } else if (getCookie('designMode') === '1'){ // ici on est en lightmode
      getLightMode();
    } else {
      console.log('error');
    };
